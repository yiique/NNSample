...
class RNNencoder(object):
    ...
    def __init__(self):
        self.cs = 5             # word window context size      4 RNN steps & 4*20 di vector
        self.ne = 21            # number of emb & input
        self.nh = 10            # number of hidden
        self.de = 10            # dimension of emb
        self.nc = 20            # number of class & output
        self.learning_rate = 0.0001
        self.window_size = 0

        try:
            f = open('RNNweight.save', 'r')
        ...
        self.get_emb = theano.function(inputs=[], outputs=self.emb)
    ...
    def train(self):
        ...
        for QApair in preprocess.QApairs:
            training_set.append(QApair['q'])
            for fact in QApair['f']:
                training_set.append(fact)

        count = 0
        for sentence in training_set:
            count += 1
            sequence = [preprocess.generate_word_index(x)+1 for x in sentence.split(' ')]

            idxs = self.generate_idxs(sequence[:-1])
            y = sequence[-1] - 1
            # print idxs, y
            ...
            if count % 100 == 0:
                ...
                print "sentence:", sentence, y
                print "y_pred:", preprocess.dictionary[self.theano_classify(idxs)]
